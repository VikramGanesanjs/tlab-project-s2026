...
class SSLMetaArch(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        if self.cfg.dino.reweight_dino_local_loss:
            ...
            self.dino_local_loss_schedule = linear_warmup_cosine_decay(
                ...
                warmup_iterations=schedule_cfg.warmup_iterations,
    ...
    def init_weights(self) -> None:
        ...
        if self.cfg.student.resume_from_teacher_chkpt:
            ...
            init_fsdp_model_from_checkpoint(
                ...
                strict_loading=not self.lora_enabled,
            )
            self.model_ema.load_state_dict(self.student.state_dict())
    ...
    def update_ema(self, m):
        if self.ema_params_lists is None:
            ...
            teacher_named = dict(self.model_ema.named_parameters())
            student_param_list = []
            teacher_param_list = []
            ...
            if self.lora_enabled:
                student_lora_names = {
                    name for name in student_named if any(marker in name for marker in LORA_MARKERS)
                }
                ema_lora_names = {
                    name for name in teacher_named if any(marker in name for marker in LORA_MARKERS)
                }
                missing_lora = sorted(student_lora_names - ema_lora_names)
                if missing_lora:
                    raise RuntimeError(
                        "EMA teacher is missing student LoRA parameters: "
                        + ", ".join(missing_lora[:8])
                    )
